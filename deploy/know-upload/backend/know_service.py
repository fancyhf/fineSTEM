"""
Know 内容库服务（内容节目展示子系统）

用途：扫描 content/know 目录（content-as-code），为 /api/v1/know 提供只读数据。
    ...
    def _ensure_loaded(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.exception("know_content_scan_failed root=%s", self.root)
...
_default_service: Optional[KnowContentService] = None


def get_know_content_service() -> KnowContentService:
    """FastAPI 依赖：默认指向配置的内容目录；测试可用 dependency_overrides 替换。"""
    global _default_service
    if _default_service is None:
        from app.core.config import settings

        # 默认仓库根 content/know（…/apps/backend/app/services → 上溯 4 级到仓库根）
        root = settings.KNOW_CONTENT_DIR or (
            Path(__file__).resolve().parents[4] / "content" / "know"
        )
        _default_service = KnowContentService(root)
    return _default_service


def set_know_content_service(service: Optional[KnowContentService]) -> None:
    """测试/运维：替换或重置进程级单例。"""
    global _default_service
