...
```powershell
# 运行所有测试
.\scripts\testing\smart_test_runner.ps1 -All

# 只运行后端测试
.\scripts\testing\smart_test_runner.ps1 -BackendOnly

# 只运行前端测试
.\scripts\testing\smart_test_runner.ps1 -FrontendOnly
```
...
```cmd
# 双击运行
scripts\testing\run_playwright_with_hot_reload.cmd
```
...
| 工具 | 用途 | 命令 |
|------|------|------|
| `scripts/testing/smart_test_runner.ps1` | 智能测试运行器 | `.\scripts\testing\smart_test_runner.ps1 -All` |
| `scripts/testing/run_playwright_with_hot_reload.cmd` | Playwright 测试（热重载） | 双击运行 |
| `scripts/testing/run_complete_test_suite.cmd` | 完整测试套件（含启动服务） | 双击运行 |
