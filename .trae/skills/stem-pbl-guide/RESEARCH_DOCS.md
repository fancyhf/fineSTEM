...
- `assets/screenshots/`：至少 4 张界面截图（首页 + 关键功能页）
- `assets/logs/`：至少 1 个日志文件
- `assets/charts/` 或 `assets/results/`：至少 1 个文件
- `40_tech_report.md`、`50_final_report.md`：必须引用证据路径

不满足时，阶段门禁应返回 `needs_redo` 并列出缺失项。

---

## 5.5 正式报告输出（中文文件名 + 风格套件）

当用户要求“正式 PDF/PPT 报告”时，必须在 `docs/reports/` 输出：
...
1. 调用 `theme-factory` 展示主题并确认主题名。
2. 将主题写入 `docs/reports/report_theme.json`（字段 `themeName`）。
3. 调用 `pptx` 流程生成演示稿并应用所选主题字体/配色。
4. 导出 DOC/DOCX/PDF/PPTX 全套正式报告。

可并行保留英文文件名版本，但中文文件名版本必须存在，且正文标题/子标题/要点必须为中文。
