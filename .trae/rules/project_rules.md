...
### 1.6 服务端口管理规范（强制）
- **端口白名单**：fineSTEM 项目所有服务必须使用以下预定义端口，禁止擅自使用白名单之外的端口：
  | 服务 | 端口 | 说明 |
  |------|------|------|
  | 后端 API (Backend) | **3000** | 主后端服务端口 |
  | 前端开发服务器 (Frontend Dev) | **5173** | Vite 开发服务器 |
  | 前端 Web (Local) | **5174** | 本地前端 Web 服务 |
  | 前端访问 (Nginx/部署) | **8081** | 部署后前端访问入口 |
  | 后端 API (部署) | **8001** | 部署后后端 API 入口 |
  | 项目服务 (Projects 等) | **4000–4999** | 子项目/微服务预留范围 |
  | 其他预留 | **5273、4500** | 特定场景预留端口 |
- **变更审批**：如需新增或调整端口，必须提交变更申请（含原因、影响范围、兼容策略、负责人与截止日期），评审通过后更新本规范与相关配置。
- **配置一致性**：所有 `.env`、`.env.example`、`vite.config.ts`、Docker 端口映射、CI/CD 脚本中的端口配置必须与上表保持一致；禁止代码中硬编码未登记的端口。
- **冲突检测**：本地启动前需检查端口占用情况；若白名单端口被占用，优先排查并释放，而非切换到未登记端口。
- **文档同步**：端口变更后必须同步更新 `apps/backend/.env.example`、`apps/frontend/vite.config.ts` 及本规范文件。
