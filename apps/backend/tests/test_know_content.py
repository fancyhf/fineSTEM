...
用途：验证 content/know 扫描、筛选语义（audience/tag/q）、主打位解析、API 只读接口
维护者：AI Agent
"""
...
from app.api.know import get_know_content_service
from app.services.know_content import KnowContentService
...
@pytest.fixture()
def svc(content_dir: Path) -> KnowContentService:
    return KnowContentService(content_dir)


class TestKnowContentService:
    def test_home_featured_and_tags(self, svc: KnowContentService):
        home = svc.home()
    ...
    def test_episode_urls(self, svc: KnowContentService):
    ...
    def test_filter_audience_parent_includes_family(self, svc: KnowContentService):
        eps = svc.list_episodes(audience="parent")
        # family 的「搭积木」与 parent 的「光的旅程」都命中
        assert sorted(e.title for e in eps) == ["光的旅程", "搭积木"]

    def test_filter_audience_child_excludes_parent(self, svc: KnowContentService):
        eps = svc.list_episodes(audience="child")
        # 亲子集命中，家长向集不命中
        assert [e.audience for e in eps] == ["family"]

    def test_filter_q_and_tag(self, svc: KnowContentService):
    ...
    def test_series_detail_and_prev_next(self, svc: KnowContentService):
    ...
    def test_missing_content_dir_is_empty(self, tmp_path: Path):
        empty = KnowContentService(tmp_path / "not-exist")
...
def client(content_dir: Path):
    test_svc = KnowContentService(content_dir)
    app.dependency_overrides[get_know_content_service] = lambda: test_svc
    with TestClient(app) as c:
        yield c
    app.dependency_overrides.pop(get_know_content_service, None)


class TestShowApi:
    def test_home_endpoint(self, client: TestClient):
        r = client.get("/api/v1/know/home")
    ...
    def test_series_endpoints(self, client: TestClient):
        assert client.get("/api/v1/know/series").status_code == 200
        r = client.get("/api/v1/know/series/recursive-beauty")
        assert r.status_code == 200
        assert r.json()["data"]["title"] == "递推之美"
        assert client.get("/api/v1/know/series/nope").status_code == 404

    def test_episodes_endpoint_filters(self, client: TestClient):
        r = client.get("/api/v1/know/episodes", params={"q": "光"})
        assert [e["title"] for e in r.json()["data"]] == ["光的旅程"]
        r = client.get("/api/v1/know/episodes", params={"audience": "child"})
        assert [e["audience"] for e in r.json()["data"]] == ["family"]
        r = client.get("/api/v1/know/episodes", params={"audience": "bad"})
        assert r.status_code == 422

    def test_episode_detail_404(self, client: TestClient):
        assert client.get("/api/v1/know/episodes/recursive-beauty/ep02").status_code == 404
        assert client.get("/api/v1/know/episodes/a/b").status_code == 404

    def test_episode_detail_embed_video(self, client: TestClient):
        r = client.get("/api/v1/know/episodes/finestem-class/ep01")
        data = r.json()["data"]
