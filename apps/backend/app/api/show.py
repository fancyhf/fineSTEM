...
用途：Show 频道（show.wostemstudio.site）的只读公开接口，数据来自
     content/show 内容目录（content-as-code），无用户态、无写操作。
维护者：AI Agent
...
from app.schemas.show import EpisodeDetail, EpisodeSummary, SeriesDetail, SeriesSummary, ShowHome
from app.services.show_content import ShowContentService, get_show_content_service

router = APIRouter(prefix="/show", tags=["节目频道"])


@router.get("/home", response_model=ApiResponse[ShowHome])
async def show_home(svc: ShowContentService = Depends(get_show_content_service)):
...
async def list_series(svc: ShowContentService = Depends(get_show_content_service)):
    """系列列表。"""
    return ApiResponse(data=svc.list_series())


@router.get("/series/{slug}", response_model=ApiResponse[SeriesDetail])
async def get_series(slug: str, svc: ShowContentService = Depends(get_show_content_service)):
    """系列详情（含集列表）。"""
...
async def list_episodes(
    ...
    svc: ShowContentService = Depends(get_show_content_service),
):
