...
export function Stage03Constraints({ value, onChange }: StageComponentProps<Stage03Payload>) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      <div>
        <label className="block text-sm text-gray-700 mb-1">Must-have</label>
        <textarea
          ...
          placeholder="Must deliver"
        />
      </div>
      <div>
        <label className="block text-sm text-gray-700 mb-1">Nice-to-have</label>
        <textarea
          value={joinLines(value.nice_to_have)}
          onChange={(e) => updateField(value, 'nice_to_have', splitLines(e.target.value), onChange)}
          className="w-full min-h-28 p-3 border border-gray-300 rounded-lg"
          placeholder="Optional if time allows"
        />
      </div>
      <div>
        <label className="block text-sm text-gray-700 mb-1">Won't-do</label>
        <textarea
