...
test.describe('核心用户闭环：Demo → Fork → 轻项目 → 成果卡 → 分享', () => {
  ...
  test('研学项目页面加载', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/research');
    await authenticatedPage.waitForLoadState('domcontentloaded');
  ...
  test('互联页面加载', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/connect');
    await authenticatedPage.waitForLoadState('domcontentloaded');
  ...
  test('课程库页面加载', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/course-library');
    await authenticatedPage.waitForLoadState('domcontentloaded');
  ...
  test('代码工作室页面加载', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/code-studio');
    await authenticatedPage.waitForLoadState('domcontentloaded', { timeout: 20000 }).catch(() => {});
...
test.describe('完整 API 驱动流程验证', () => {
  ...
  test('通过 API 完成完整闭环并验证前端展示', async ({ page, testUser }) => {
    ...
    if (demos.length > 0) {
      ...
      // 6. 访问分享页面
      await page.goto(`/share/${shareData.share_token}`);
      await page.waitForLoadState('domcontentloaded');
      ...
      // 7. 验证项目详情页
      await page.goto(`/research/projects/${project.id}`);
      await page.waitForLoadState('domcontentloaded');
